      ...
      <FormItem>
        <div v-for="(btn) in btnGroup" style="display:inline" :key="btn.key">
          <Button
            ...
            :type="btn.type"
            :style="btn.style"
            @click="handleOnclickProxy(btn)"
          >{{typeof(btn.label)==="function"?btn.label(detail):btn.label}}</Button>
        </div>
      </FormItem>
    </Form>
  </div>
  ...
  data() {
    ...
    if (this.formDataEntity) {
      ...
      if (this.iseditor) {
        this.formDataEntity.map(obj => {
          if (obj.type === "group") {
            let groups = typeof obj.data === "function" ? obj.data(this.detail) : [];

            let showFlag = true;
            if (groups.length === 1)
              obj.groups.map(o => {
                geneData[o.prop] = groups[0][o.prop];
              });

            if (!obj.show(geneData)){
            } else {
              groups.map(group => {
                let gro = {};
                obj.groups.map(o => {
                  geneData[o.prop]=""
                  gro[o.prop] =
                    typeof o.data === "function"
                      ? o.data(group[o.prop])
                      : group[o.prop];
                });
                geneData[obj.prop].push(gro);
              });
            }
          } else {
            geneData[obj.prop] =
              typeof obj.data === "function"
                ? obj.data(this.detail[obj.prop])
                : this.detail[obj.prop];
          }
        });
            ...
            if (obj.show(this.data)) {
              ...
              if (items.length == 0) {
                ...
                obj.groups.map(o => {
                  ...
                  if (!this.data[o.prop]) emptyCount += 1;
                  memberCount += 1;
                });
